...
function Recount.SettingsMenu:Setup()
	local optionsData = {
		...
		},
		{
			...
			tooltip = "Heals below this value will *NOT* show up in the log, but will count as HPS in total",
			getFunc = function() return Recount.settings.minLogValue end,
			setFunc = function(value) Recount.settings.minLogValue = value end,
			isMultiline = false,
			default = Recount.settingsDefaults.minLogValue,
		},
	}
